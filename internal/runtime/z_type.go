...
func newPointer(elem *Type) *Type {
	if t := rtypeList.findElem(abi.Pointer, elem, 0); t != nil {
		return t
	}
	ptr := &abi.PtrType{
		Type: Type{
...
func ArrayOf(length uintptr, elem *Type) *Type {
	...
	ret := &abi.ArrayType{
		Type: Type{
			...
			Str_: "[" + string(itoa(make([]byte, 20), uint64(length))) + "]" +
				elem.String(),
		},
		Elem:  elem,
		Slice: SliceOf(elem),
...
func (r *rtypes) findNamed(name string) *Type {
	for _, typ := range r.list {
		if typ.HasName() && typ.named == name {
			return typ.Type
		}
	}
	return nil
...
func (r *rtypes) addNamed(typ *Type, name string) {
	r.list = append(r.list, &rtype{Type: typ, named: name})
}
...
func (r *rtypes) findInterface(pkgPath string, name string, methods []Imethod) *abi.InterfaceType {
	for _, typ := range r.list {
		if typ.Kind() == abi.Interface && typ.Str_ == name {
...
func (r *rtypes) addType(typ *Type) {
	r.list = append(r.list, &rtype{Type: typ})
}
