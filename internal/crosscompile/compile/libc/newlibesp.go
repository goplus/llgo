...
func getNewlibESP32ConfigRISCV(baseDir, target string) compile.CompileConfig {
	...
	groups = append(groups, getNewlibESP32RISCVLibmCompileGroups(baseDir, target, libcDir)...)
...
func getNewlibESP32LibmFiles(baseDir string) []string {
	files := make([]string, 0, 512)
	for _, group := range newlibESP32RISCVLibmFileGroups {
		for _, rel := range group.relativeFiles {
			files = append(files, filepath.Join(baseDir, "newlib", rel))
		}
	}
	return files
}

func getNewlibESP32LibmFlagKey(flags []string) string {
	return strings.Join(flags, "\x00")
}

func getNewlibESP32LibmFlagSuffix(flags []string) string {
	if len(flags) == 0 {
		return "default"
	}
	parts := make([]string, 0, len(flags))
	for _, flag := range flags {
		part := strings.TrimLeft(flag, "-")
...
func getNewlibESP32RISCVLibmLDFlags() []string {
	ldflags := append([]string{}, _libcLDFlags...)
	// newlib/README (--enable-newlib-nano-formatted-io) says float I/O in
	// nano-vfprintf is split into weak symbols and must be explicitly linked
	// with `-u _printf_float` (or `_scanf_float`).
	ldflags = append(ldflags, "-u", "_printf_float")
	return ldflags
}

func getNewlibESP32RISCVLibmCompileGroups(baseDir, target, libcDir string) []compile.CompileGroup {
	baseCFlags := []string{
		"-DHAVE_CONFIG_H",
		"-D_LIBC",
		"-DHAVE_NANOSLEEP",
	...
	libmLDFlags := getNewlibESP32RISCVLibmLDFlags()

	type libmBucket struct {
		extraCFlags []string
		files       []string
	}

	buckets := make(map[string]*libmBucket, len(newlibESP32RISCVLibmFileGroups))
	order := make([]string, 0, len(newlibESP32RISCVLibmFileGroups))
	for _, group := range newlibESP32RISCVLibmFileGroups {
		key := getNewlibESP32LibmFlagKey(group.extraCFlags)
		bucket, ok := buckets[key]
		if !ok {
			bucket = &libmBucket{
				extraCFlags: append([]string{}, group.extraCFlags...),
			}
			buckets[key] = bucket
			order = append(order, key)
		}
		for _, rel := range group.relativeFiles {
			bucket.files = append(bucket.files, filepath.Join(baseDir, "newlib", rel))
		}
	}

	groups := make([]compile.CompileGroup, 0, len(order))
	suffixCount := make(map[string]int, len(order))
	for _, key := range order {
		bucket := buckets[key]

		baseSuffix := getNewlibESP32LibmFlagSuffix(bucket.extraCFlags)
		suffix := baseSuffix
		if n := suffixCount[baseSuffix]; n > 0 {
			suffix = fmt.Sprintf("%s_%d", baseSuffix, n+1)
		}
		suffixCount[baseSuffix]++

		cflags := append([]string{}, baseCFlags...)
		cflags = append(cflags, bucket.extraCFlags...)

		groups = append(groups, compile.CompileGroup{
			OutputFileName: fmt.Sprintf("libm-%s-%s.a", suffix, target),
			Files:          append([]string{}, bucket.files...),
			CFlags:         cflags,
