...
func getNewlibESP32ConfigRISCV(baseDir, target string) compile.CompileConfig {
	...
	libmFBuiltinFiles := make([]string, 0, len(libmFileGroups[0].relativeFiles))
	for _, rel := range libmFileGroups[0].relativeFiles {
		libmFBuiltinFiles = append(libmFBuiltinFiles, filepath.Join(baseDir, "newlib", rel))
	}
	libmDefaultFiles := make([]string, 0, len(libmFileGroups[1].relativeFiles))
	for _, rel := range libmFileGroups[1].relativeFiles {
