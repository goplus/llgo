...
func getNewlibESP32ConfigRISCV(baseDir, target string) compile.CompileConfig {
	...
	libmFBuiltinFiles := make([]string, 0, len(newlibESP32RISCVLibmFileGroups[0].relativeFiles))
	for _, rel := range newlibESP32RISCVLibmFileGroups[0].relativeFiles {
		libmFBuiltinFiles = append(libmFBuiltinFiles, filepath.Join(baseDir, "newlib", rel))
	}
	libmDefaultFiles := make([]string, 0, len(newlibESP32RISCVLibmFileGroups[1].relativeFiles))
	for _, rel := range newlibESP32RISCVLibmFileGroups[1].relativeFiles {
		libmDefaultFiles = append(libmDefaultFiles, filepath.Join(baseDir, "newlib", rel))
	}
