...
type evp_md_st struct {
	...
	Flags     c.Ulong
	Origin    c.Int
	Init      func(ctx *EVP_MD_CTX) c.Int
	Update    func(ctx *EVP_MD_CTX, data unsafe.Pointer, count uintptr) c.Int
	Final     func(ctx *EVP_MD_CTX, md *byte) c.Int
	Copy      func(to *EVP_MD_CTX, from *EVP_MD_CTX)
	Cleanup   func(ctx *EVP_MD_CTX)
	BlockSize c.Int
	CtxSize   c.Int // how big does the ctx->md_data need to be
	MdCtrl    func(ctx *EVP_MD_CTX, cmd c.Int, p1 c.Int, p2 unsafe.Pointer)
