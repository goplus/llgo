...
func main() {
	...
	var digest = make([]byte, evp.EVP_MAX_MD_SIZE)
	...
	defer openssl.FreeHMAC_CTX(ctx)

	var ret c.Int = ctx.InitEx(unsafe.Pointer(c.Str(key)), c.Int(lenKey), openssl.EVP_sha256(), nil)
	if ret == 0 {
