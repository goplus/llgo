...
- 若决定以 **IR/BC pass 为唯一入口**（无论是源码还是包复用场景），应避免再解析 Go obj 的重定位体系：  
  - 会形成“双体系”（Go obj reloc + IR 标记）并导致模型不一致、维护成本上升。  
  - LLVM IR 本就显式表达普通调用/引用边，结合 `@__llgo_relocs` 足以完成“普通依赖 + Go 语义依赖”的统一分析。
- 推荐分工：  
...
### 补充方案：随 .o 携带 bitcode，合并后跑 pass 再生成最终 .o
- 为自家产出的 .o 携带或旁带 bitcode（类似胖对象/ThinLTO 思路），在最终链接前抽取所有可用 bitcode 合并为一个 Module，跑自定义 pass（可达性 + 哨兵回填），再生成新的 .o 给 lld 链接。
- 没有 bitcode 的外部 .o/.a 视为黑盒依赖，不做裁剪；这类输入本就与 Go 语义无关，供 C 生态复用时自然不会带上 Go 特有标记。
- 价值：让“自家”代码的 Go 语义裁剪在全局视角下完成，而不依赖 lld 理解标记；限制：第三方纯 .o 依然不可改写。
