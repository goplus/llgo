...
type setting struct {
	...
	nonDefaultMu     sync.Mutex
	nonDefaultInited bool
...
func (s *Setting) IncNonDefault() {
	s.nonDefaultMu.Lock()
	if !s.nonDefaultInited {
		s.register()
		s.nonDefaultInited = true
	}
	s.nonDefaultMu.Unlock()
	s.nonDefault.Add(1)
}
