...
func (b Builder) goCoro(fn Expr, args ...Expr) {
	...
	// Get the function name for dual-symbol mode
	fnName := ""
	if fn.kind != vkBuiltin && !fn.impl.IsNil() {
		fnName = fn.impl.Name()
	}

	// Must be a named function (not closure) for dual-symbol mode
	...
	// Look up the $coro version (pre-generated by compileCoroFuncDecl)
...
// In dual-symbol mode, the $coro version is pre-generated by compileCoroFuncDecl.
// For cross-package calls, declares the function (body is in the other package).
func (p Package) getOrCreateCoroFunc(fnName string, origFn Expr) Expr {
	...
	// Look up or declare the $coro function (body is pre-generated or in another package)
