...
func (b Builder) Defer(kind DoAction, fn Expr, args ...Expr) {
	...
	self.stmts = append(self.stmts, func(ptr Expr, bits Expr) {
		switch kind {
		case DeferInCond:
			zero := prog.Val(uintptr(0))
...
func (p Function) endDefer(b Builder) {
	...
	for i := n - 1; i >= 0; i-- {
		...
		ptr := b.IndexAddr(b.Load(self.argsPtr), b.Prog.Val(i))
		stmts[i](b.Load(ptr), b.Load(bitsPtr))
		if i != 0 {
			b.Jump(rethNext)
		}
	}
