...
func TestNamedStructEquivalentReuse(t *testing.T) {
	src1 := types.NewPackage("bar", "foo/bar")
	src2 := types.NewPackage("bar", "foo/bar")
	t1 := types.NewNamed(
		types.NewTypeName(0, src1, "Value", nil),
		types.NewStruct([]*types.Var{
			types.NewField(0, src1, "n", types.Typ[types.Int], false),
		}, nil),
		nil,
	)
	t2 := types.NewNamed(
		types.NewTypeName(0, src2, "Value", nil),
		types.NewStruct([]*types.Var{
			types.NewField(0, src2, "n", types.Typ[types.Int], false),
		}, nil),
		nil,
	)
	...
	ir := pkg.Module().String()
	if !strings.Contains(ir, "@a = external global %bar.Value") || !strings.Contains(ir, "@b = external global %bar.Value") {
		t.Fatalf("equivalent named types should share LLVM name:\n%s", ir)
	}
	if strings.Contains(ir, "bar.Value#0") {
		t.Fatalf("equivalent named types unexpectedly disambiguated:\n%s", ir)
	}
}
